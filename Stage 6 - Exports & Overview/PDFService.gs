 ...
 * PDFService.gs — PDF generation for result slips (Stage 5 → updated Stage 6)
 *
 * Generates result slip PDFs for individual students or all active students
 * in a class group in one combined document.
 *
 * ─── STAGE 6 CHANGES (do NOT modify Stage 5 version — replace with this file) ──
 *
 *   1. LOGO IN LETTERHEAD
 *      The school logo is now fetched via ExportService.fetchLogo() and embedded
 *      in the letterhead of every result slip. If the logo is unavailable (Drive
 *      not yet authorized, or SCHOOL_LOGO_FILE_ID not set in ExportService), the
 *      header renders without it — identical to Stage 5 behaviour.
 *
 *   2. PAGE MARGIN
 *      @page margin changed from "1cm 1.2cm" → "1cm" (equal on all sides).
 *
 * ─── ALL OTHER CODE IS IDENTICAL TO STAGE 5 ─────────────────────────────────
 *
...
const PDFService = (function () {
   ...
   *
   * STAGE 6 CHANGE: The school logo is now fetched from ExportService.fetchLogo()
   * and embedded in the letterhead. If ExportService is unavailable or the logo
   * fetch fails, the slip renders without logo (graceful fallback).
   *
  ...
  function buildSlipHtml_(result, isLastStudent) {
    ...
    // ── Page wrapper ──────────────────────────────────────────────────────
    // padding:0 — the @page margin (1cm) already handles all four sides equally.
    // Removing the original padding:20px 0 keeps the top margin consistent with
    // the left and right margins set in @page.
    var pageBreak = isLastStudent ? '' : 'page-break-after:always;';
    var html = '<div style="' + pageBreak + 'font-family:Arial,Helvetica,sans-serif;font-size:10pt;color:' + COLOR_TEXT + ';width:720px;margin:0 auto;padding:0;">';

    // ── LETTERHEAD ────────────────────────────────────────────────────────
    //
    // Stage 6 update: Logo is fetched from ExportService.fetchLogo() and
    // rendered as an <img> cell on the left of the header bar.
    //
    // Why call ExportService here instead of DriveApp directly?
    // ExportService already owns the SCHOOL_LOGO_FILE_ID constant and the
    // Drive fetch + base64 encoding logic. Calling it here avoids duplicating
    // that logic and keeps the logo file ID in one place.
    //
    // If ExportService is undefined (e.g. file not deployed yet) or the fetch
    // throws for any reason, logoData stays null and the header renders without
    // an image — identical to Stage 5 behaviour.
    var logoData = null;
    try {
      if (typeof ExportService !== 'undefined' && ExportService.fetchLogo) {
        logoData = ExportService.fetchLogo();
      }
    } catch (logoErr) {
      Logger.log('PDFService: logo fetch failed — ' + logoErr.message);
    }

    // ── LETTERHEAD — logo centered above school name ─────────────────────
    // Logo sits above the school name in a single centered column.
    // This mirrors the professional layout of a printed letterhead where
    // the crest/seal is at the top, centered, with the school name below it.
    html += '<table style="width:100%;border-collapse:collapse;background:' + COLOR_BG_HEADER + ';" cellpadding="0" cellspacing="0"><tr>';
    html += '<td style="text-align:center;padding:12px 12px 10px 12px;">';

    // Logo (if available) above the school name
    if (logoData && logoData.dataUri) {
      html += '<div style="margin-bottom:6px;">';
      html += '<img src="' + logoData.dataUri + '" style="height:56px;width:auto;object-fit:contain;">';
      html += '</div>';
    }
    ...
    html += '<span style="font-weight:bold;color:' + COLOR_MUTED + ';">Name:</span> <span style="font-weight:bold;">' + escapeHtml_(student.name) + '</span>';
    html += '</td>';
   ...
   *
   * STAGE 6 CHANGE: @page margin changed from "1cm 1.2cm" → "1cm" (equal on all sides).
   *
  ...
  function wrapDocument_(bodyContent, title) {
    return (
      ...
      '<style>' +
        ...
        // Stage 6: margin reduced to 1cm equal on all sides (was 1cm 1.2cm).
        '@page { margin: 1cm; }' +
  ...
  function generateResultSlipPDF(token, studentId, classId) {
    ...
    var result = assembleStudentResult_(
      ...
      gd.fmNameByClass[classId] || '', studentName
    );
  ...
  function generateBulkResultsPDF(token, classGroupKey) {
    ...
    gd.allGroupStudents.forEach(function (studentObj, idx) {
      ...
      var result = assembleStudentResult_(
        ...
        gd.allGroupStudents, gd.activeStudentIds, gd.subjectIds, gd.subjectById,
        gd.allStatusMap, gd.psqLookup, gd.remarkLookup,
        gd.fmNameByClass[studentObj.classId] || '', studentObj.name
      );
