...
// ═══════════════════════════════════════════════════════════════════════════
// STAGE 7 PATCH — Add slipName (SURNAME-FIRST format) to result slip data
// ═══════════════════════════════════════════════════════════════════════════
//
// Result slips should display the student name as "SMITH David Obi"
// (surname in ALL CAPS, then first/middle name in normal case).
//
// This IIFE wraps getStudentResult: after the original function assembles
// the result, it looks up the student's lastName and firstMiddleName from
// the Students Cache and adds a `slipName` field to the response.
//
// `student.name` (fullName, first-last order) is left UNCHANGED so that
// all other views (score entry, broadsheet, class list) are unaffected.
// Only renderResultSlip in AppScript.html reads `slipName`.
//
// HOW TO MODIFY:
//   - To change the format (e.g. add a comma), edit the slipName line below.
//   - To revert to fullName on result slips, delete this entire IIFE block.
// ═══════════════════════════════════════════════════════════════════════════

(function () {

  var _originalGetStudentResult = ResultService.getStudentResult;

  ResultService.getStudentResult = function (token, studentId, classId) {

    // Run the original function to get the full result data.
    var result = _originalGetStudentResult(token, studentId, classId);

    // Only augment successful responses that have a student object.
    if (!result || !result.success || !result.data || !result.data.student) {
      return result;
    }

    // Look up the cached student to get the stored name components.
    // These are populated by StudentService.refreshStudentCache (Stage 7 patch).
    try {
      var students = SheetService.getCachedStudents(classId);
      var cached   = students.filter(function (s) {
        return String(s.studentId) === String(studentId);
      })[0];

      var slipName;
      if (cached && cached.lastName && cached.lastName.trim() !== '') {
        // Format: SURNAME FirstMiddle — e.g. "SMITH David Obi"
        var surname     = String(cached.lastName        || '').trim().toUpperCase();
        var firstMiddle = String(cached.firstMiddleName || '').trim();
        slipName = surname + (firstMiddle ? ' ' + firstMiddle : '');
      } else {
        // Fallback: cache hasn't been refreshed yet with the new columns —
        // use the existing fullName so the result slip still shows something.
        slipName = result.data.student.name || '';
      }

      result.data.student.slipName = slipName;

    } catch (e) {
      // Never let a logging/augmentation error break result slip loading.
      result.data.student.slipName = result.data.student.name || '';
      Logger.log('ResultService Stage 7 patch: slipName failed: ' + e.message);
    }
