 ...
 *   real class data. This file creates a small dummy class (TEST_CLS001) with
 *   3 dummy students and 2 dummy subjects, then gives you individual functions
 *   to progressively unlock the Broadsheet and Results buttons.
 *
 * ─── HOW TO USE ───────────────────────────────────────────────────────────────
 *
 *   STEP 1 — Add this file to your GAS project (temporary).
 *             Click + → Script → name it TestDataHelper → paste this file → Save.
 *
 ...
 *             Open a student's result slip → click "Download PDF".
 *             Go back to the student list → click "Generate All PDFs".
 *
 *   STEP 5 — Run cleanupTestData() to remove everything.
 *             Then delete this file from the GAS project.
 *
 * ─── WHAT GETS CREATED ────────────────────────────────────────────────────────
 *
 ...
 *   Class-Subject Assignments: 2 rows (TEST_CLS001 × each test subject, current session)
 *
...
var TEST_STUDENTS = [
  { studentId: 'TEST_STU001', fullName: 'Alpha Testson',   gender: 'Male'   },
  { studentId: 'TEST_STU002', fullName: 'Beta Tester',     gender: 'Female' },
  { studentId: 'TEST_STU003', fullName: 'Gamma Checklist', gender: 'Male'   }
];
...
function seedTestData() {
  ...
  TEST_STUDENTS.forEach(function (s) {
    if (existingStudentIds.indexOf(s.studentId) === -1) {
      ...
      Logger.log('[seedTestData] Created student: ' + s.studentId + ' — ' + s.fullName);
    } else {
  ...
  // ── Class-Subject Assignments (scoped to the active session) ──────────────
...
function seedTestPSQ() {
  ...
  TEST_STUDENTS.forEach(function (s) {
    ...
    Logger.log('[seedTestPSQ] Inserted PSQ for ' + s.studentId + ' (' + s.fullName + ')');
  });
...
function seedTestRemarks() {
  ...
  TEST_STUDENTS.forEach(function (s) {
    ...
    Logger.log('[seedTestRemarks] Inserted remark for ' + s.studentId + ' (' + s.fullName + ')');
  });
 ...
 *
 * After running: delete this file from the GAS project.
 */
...
function verifyTestData() {
  ...
  TEST_STUDENTS.forEach(function (s) {
    var found = studentRows.some(function (r) { return String(r['Student ID']) === s.studentId; });
    Logger.log('Student:  ' + s.studentId + ' (' + s.fullName + ')  ' + (found ? '✓' : 'NOT FOUND ✗ — run seedTestData()'));
  });
